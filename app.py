...
def login_with_oauth_screen(login_form_placeholder):
...
def login_with_otp_screen(login_form_placeholder):
...
def main():
    ...
    #     login_with_otp_screen(login_form_placeholder)

    # Login with oauth
    if not st.user.is_logged_in:
        login_with_oauth_screen(login_form_placeholder)

    else:
        login_form_placeholder.empty()
        # if st.session_state.authenticated_user:
        #     st.success(f'Welcome, {st.session_state.authenticated_user}!')
        #     logout_button = st.button(label='Logout', on_click=st.on_logout_click)
        if st.user.name:
